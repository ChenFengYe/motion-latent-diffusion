...
class UESTCMetrics(Metric):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # load pretrianed
        model_path = os.path.join(cfg.MODEL.UESTC_REC_PATH, "uestc_rot6d_stgcn.tar")
        ...
        self.add_state("gt_accuracy", default=torch.tensor(0.0), dist_reduce_fx="mean")
        self.metrics.extend(["accuracy", "gt_accuracy"])
        # Fid
        self.add_state("FID", default=torch.tensor(0.0), dist_reduce_fx="mean")
        self.add_state("gt_FID", default=torch.tensor(0.0), dist_reduce_fx="mean")
        ...
        # Multimodality
        self.add_state(
            "Multimodality", default=torch.tensor(0.0), dist_reduce_fx="mean"
        )
        self.add_state(
            "gt_Multimodality", default=torch.tensor(0.0), dist_reduce_fx="mean"
        )
